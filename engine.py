class Game:
    ...
    def __init__(self):
        ...
        self.left_white_castling = False # рокировка ладьи
        self.right_white_castling = False
        self.left_black_castling = False
        self.right_black_castling = False

        self.rock_left_white = False # помощь для хода - рокировки
        self.rock_right_white = False
        self.rock_left_black = False
        self.rock_right_black = False
    ...
    def get_rook_moves(self, r, c): # return возможные ходы ладьи
        # возвращаемое значение
        m = []
    ...
    def get_king_moves(self, r, c):
        ...
        if self.board[r][c][1] == 'K':
            if self.whiteToMove and self.board[r][c][0] == 'w':
                ...
                if not self.white_castling:
                    # левая сторона
                    if self.board[r][c - 1] == '--' and self.board[r][c - 2] == '--' and self.board[r][c - 3] == 'wR':
                        if not self.left_white_castling:
                            m.append(['cst', r, c - 2])
                    # правая сторона
                    if self.board[r][c + 1] == '--' and self.board[r][c + 2] == '--' and self.board[r][c + 3] == '--' and self.board[r][c + 4] == 'wR':
                        if not self.right_white_castling:
                            m.append(['cst', r, c + 2])
                ...
                if not self.black_castling:
                    # левая сторона
                    if self.board[r][c - 1] == '--' and self.board[r][c - 2] == '--' and self.board[r][c - 3] == 'bR':
                        if not self.left_black_castling:
                            m.append(['cst', r, c - 2])
                    # правая сторона
                    if self.board[r][c + 1] == '--' and self.board[r][c + 2] == '--' and self.board[r][c + 3] == '--' and self.board[r][c + 4] == 'bR':
                        if not self.right_black_castling:
                            m.append(['cst', r, c + 2])
    ...
    def make_move(self, start_sq, end_sq): # совершение хода
        ...
        if self.runTrue and self.pieceMoved[0] != '-':
            ...
            if self.whiteToMove and self.pieceMoved[0] == 'w':
                ...
                if r:
                    # если черному королю шах и ход белых => то белые выиграли
                        ...
                        for i in h:
                            if i[0] == 'get':
                                ...
                                if self.pieceCaptured != '--':
                                    if self.endRow == i[1] and self.endCol == i[2]:
                                        self.make(self.startRow, self.startCol, self.endRow, self.endCol, self.pieceMoved, self.pieceCaptured)
                            elif i[0] == 'cst' and self.pieceMoved[1] == 'K':
                                if self.endRow == i[1] and self.endCol == i[2]:
                                    if self.startRow == 7:
                                        ...
                                        if self.endCol == 1:
                                            #print('left white')
                                            if not self.rock_left_white:
                                                n = True
                                                self.make(7, 0, 7, 2, 'wR', '--', castling_r=True)
                                        elif self.endCol == 5:
                                            #print('right white')
                                            if not self.rock_right_white:
                                                n = True
                                                self.make(7, 7, 7, 4, 'wR', '--', castling_r=True)

                                        if n:
                                            self.make(self.startRow, self.startCol, self.endRow, self.endCol, self.pieceMoved, '--', castling_r=True)
                                            # False, 0 и для хода черных также выставлять в True, 0
                                        self.rock_left_white = False
                                        self.rock_right_white = False
            ...
            if not self.whiteToMove and self.pieceMoved[0] == 'b':
                ...
                if r:
                    # если белому королю шах и ход черных => то черные выиграли
                            ...
                            if i[0] == 'get':
                                ...
                                if self.endRow == i[1] and self.endCol == i[2]:
                                    if self.startRow == 0:
                                        ...
                                        if self.endCol == 1:
                                            #print('left black')
                                            if not self.rock_left_black:
                                                n = True
                                                self.make(0, 0, 0, 2, 'bR', '--', castling_r=True)
                                        elif self.endCol == 5:
                                            #print('right black')
                                            if not self.rock_right_black:
                                                n = True
                                                self.make(0, 7, 0, 4, 'bR', '--', castling_r=True)
    ...
    def make(self, startRow, startCol, endRow, endCol, pieceMoved, pieceCaptured, castling_r=False):
        ...
        if pieceMoved[1] == "R":
            if pieceMoved[0] == "w":
                if startRow == 7 and startCol == 0:
                    self.left_white_castling = True
                    self.f[2] = True
                elif startRow == 7 and startCol == 7:
                    self.right_white_castling = True
                    self.f[3] = True
            elif pieceMoved[0] == "b":
                if startRow == 0 and startCol == 0:
                    self.left_black_castling = True
                    self.f[4] = True
                elif startRow == 0 and startCol == 7:
                    self.right_black_castling = True
                    self.f[5] = True

        if castling_r:
            self.moveLog.append([startRow, startCol, endRow, endCol, pieceMoved, pieceCaptured, castling_r, self.f])
        else:
            self.moveLog.append([startRow, startCol, endRow, endCol, pieceMoved, pieceCaptured, False, self.f])
        self.f = [False] * 6
        ...
        if len(self.moveLog) != 0:
            ...
            if any(last[7]):
                if last[7][0]:
                    ...
                    self.left_white_castling = False
                elif last[7][3]:
                    self.right_white_castling = False
                elif last[7][4]:
                    self.left_black_castling = False
                    ...
                    self.right_black_castling = False
            if last[6]:
                last2 = self.moveLog.pop()
                self.board[last2[0]][last2[1]] = last2[4]
                self.board[last2[2]][last2[3]] = last2[5]
                if any(last2[7]):
                    if last2[7][0]:
                        self.white_castling = False
                    elif last2[7][1]:
